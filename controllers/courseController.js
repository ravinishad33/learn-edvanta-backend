...
const { deleteFromCloudinary, deleteMultipleFromCloudinary } = require("../utils/cloudinary");
...
const updateCourse = async (req, res) => {
  try {
    ...
    if (
      thumbnail?.publicId &&
      thumbnail.publicId !== existingCourse.thumbnail?.publicId
    ) {
      await deleteFromCloudinary(existingCourse.thumbnail.publicId);
    }

    // CASE 1: New promo video provided → replace old
    if (
      promotionalVideo?.publicId &&
      promotionalVideo.publicId !== existingCourse.promotionalVideo?.publicId
    ) {
      if (existingCourse.promotionalVideo?.publicId) {
        await deleteFromCloudinary(
          existingCourse.promotionalVideo.publicId,
          "video"
        );
      }
    }

    // CASE 2: Promo video removed (not sent in update)
    if (
      !promotionalVideo &&
      existingCourse.promotionalVideo?.publicId
    ) {
    ...
    const oldVideos = existingCourse.sections.flatMap(sec =>
      sec.lessons
        .filter(lesson => lesson.video?.publicId)
        .map(lesson => lesson.video.publicId)
    );

    // If sections NOT provided → delete all old videos
    if (!sections) {
      if (oldVideos.length) {
        await deleteMultipleFromCloudinary(oldVideos, "video");
      }
    } else {
      // Collect new videos
      const newVideos = sections.flatMap(sec =>
        sec.lessons
          ?.filter(lesson => lesson.video?.publicId)
          .map(lesson => lesson.video.publicId)
      );
      ...
      // Find removed videos
      const videosToDelete = oldVideos.filter(id => !newVideos.includes(id));

      // Delete only removed ones
      if (videosToDelete.length) {
        await deleteMultipleFromCloudinary(videosToDelete, "video");
      }
    }
...
const deleteCourse = async (req, res) => {
  try {
    ...
    // 2. SECURITY: Check if the user is an Admin OR the Instructor who owns this course
    ...
    // 3. Clean up Cloudinary Files (Media)
    // Wrap in try-catch if you want the DB delete to happen even if Cloudinary fails
    try {
      if (course.thumbnail?.publicId) {
        await deleteFromCloudinary(course.thumbnail.publicId);
      }

      if (course.promotionalVideo?.publicId) {
        await deleteFromCloudinary(course.promotionalVideo.publicId, "video");
      }

      const videoIds = course.sections.flatMap(sec =>
        sec.lessons
          .map(lesson => lesson.video?.publicId)
          .filter(Boolean)
      );
      ...
      console.error("Cloudinary Cleanup Warning:", cloudinaryErr);
      // We continue deleting the DB record even if Cloudinary cleanup has a minor hiccup
    }

    // 4. DATABASE CLEANUP (Concurrent operations for speed)
    await Promise.all([
      Course.findByIdAndDelete(courseId),
      Enrollment.deleteMany({ courseId }),
      // We keep the payment record for taxes/accounting, just unlink the ID
      Payment.updateMany({ course: courseId }, { course: null }), 
      Certificate.updateMany({ course: courseId }, { course: null })
    ]);

    res.status(200).json({
      success: true,
      message: `Course "${course.title}" and its media files have been permanently removed.`,
    });
  } catch (error) {
