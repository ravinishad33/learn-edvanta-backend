...
//       path: "courseId",
//       select: "totalLessons duration certificateIssued", // include duration and certificate info
//     });
...
//             if (e.progress !== undefined) return sum + e.progress;
...
//       const duration = e.courseId?.duration || 0; // assume duration in hours
//       return sum + Number(duration);
//     }, 0);

//     // Total certificates
//     const totalCertificates = completedCourses.filter(
//       e => e.courseId?.certificateIssued
//     ).length;
...
const watchEnrolledCourse = async (req, res) => {
  try {
    ...
    const course = await Course.findById(courseId)
      .select("title description sections totalDuration instructor reviews averageRating totalRatings")
