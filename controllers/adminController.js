...
const deleteUser = async (req, res) => {
  try {
    ...
    // 1️⃣ Delete enrollments of the user
    await Enrollment.deleteMany({ studentId: userId });

    // 2️⃣ Delete certificates of the user
    await Certificate.deleteMany({ student: userId });

    // 3️⃣ Delete courses created by the user (if teacher/admin)
...
const getAdminDashboardStats = async (req, res) => {
  try {
    ...
    const avgRating = courseAgg[0]?.avgRating
      ? courseAgg[0].avgRating.toFixed(1)
      : 0;
...
const getSystemStatus = async (req, res) => {
  try {
    ...
    // const systemLogs = [
    //   { task: "Backup completed", time: "2 hours ago" },
    //   { task: "Security scan", time: "4 hours ago" },
    //   { task: "Cache cleared", time: "6 hours ago" },
    // ];
...
const getAdminCourses = async (req, res) => {
  try {
    ...
    const courses = await Course.find()
      ...
      .select(
        "title price discountPrice visibility enrolledStudents totalLessons createdAt rating status thumbnail"
      ); // Added 'status'
